...
r_squared <- runif(n_points, min = 0, max = radius^2)
...
plot(qt)
print(qt) 
# p-value is 0.48. This is above 0.05 which means that we have support in that the points are randomly distributed.
```
...
# p-value = 0.0005 which means that we can reject the null-hypothesis (that the points are not clustered), which means that the points are clustered. 
...
# p-value = 0.0016 which means that we can reject the null-hypothesis (that the points are not regular), which means that the points are distributed regularly 
```
...
# Read data
```{r}
library(readr) # libary with read_rds() function

# Load data
p1 <- read_rds("data/p1.rds")
p2 <- read_rds("data/p2.rds")

# Test if they are clustered or regular
quadrat.test(p1, alternative = "clustered") # p-value = 0.02 
quadrat.test(p2, alternative = "clustered") # p-value < 0.05

# Answers
# p1 is regular (we cannot reject the null hypothesis stating that it is non-clustered/regular)
# p2 is clustered (we can reject the null hypothesis stating that it is non-clustered/regular)
```

### Possible Answers

1. p1 is regular, p2 is regular
2. p1 is regular, p2 is clustered <--- This one
