...
# It deviates from being a Poisson process
...
# Since the function goes above the envelope in the beginning (smaller radii) it means that the points are clustered.  For larger radii, the function goes below. Hence, there are both clustering and dispersion in the points. This is often what we see for real world data.

# Repeat for regular data
K_regular_env <- envelope(p_regular, Kest, correction = "border")
plot(K_regular_env, . - pi * r ^ 2 ~ r)
# Since the function goes below the envelope in the beginning (smaller radii) it means that the points are dispersed (space out) 
```

# Inside the envelope means that the distirbution is random - the closer it is to the red line, the more it follows a random distribution.

# Red line = complete spatial randomness
# grey area: confidence internal
# Black line: empirical observations

# How the black line deviates from the red line tells us how the distributed is (above = clustered points, below = dispersion of points)
...
# Plotting tree locations
plot(redoak)

# Test
quadrat.test(redoak, alternative = "clustered")
# Answer: I can reject the null hypothesis (random uniform distribution) with p-value less than 0.01, which means that the points are clustered 
```
