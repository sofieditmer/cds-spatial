...
cities_sf <- st_as_sf(cities, coords = c("Longitude (X)", "Latitude (Y)")) # I use the st_as_sf() function to convert from a csv-file to a sf object
...
# FALSE - this means that it is not geographic but it is in fact projected
```


### Question 1: 
*What are the measurement units of the `cities_sf3035` object?*
The units are meters (m) which is evident when using st_crs on the object. 
...
plot(st_geometry(cities_sf3035)[1:10], col = "yellow") # plotting buffers
plot(st_geometry(cities_5km)[1:10], pch=20, cex = 0.1, add = TRUE) # plotting cities
...
ten_buffers <- cities_5km %>% 
  slice(1:10)

ten_cities <- cities_sf3035 %>% 
  slice(1:10)
...
tm_shape(ten_buffers)  +
  tm_polygons(col = "red") + # color = red
  tm_shape(ten_cities) +
  tm_text("Ancient Toponym", size = 1, auto.placement = 5) + # size = 1
  tm_dots(col = "yellow", # color = yellow
             size = 0.1) +
  tm_scale_bar(breaks = c(0, 10, 20), # scale breaks = 0 km., 10 km., 20 km.
               text.size = 10,
...
# Extract the latitude and longitude values with str_extract()
inscriptions$longitude <- as.numeric(str_extract(inscriptions$coordinates, "\\d+[[:punct:]]\\d+"))
inscriptions$latitude <- as.numeric(str_extract(inscriptions$coordinates, "[[:blank:]]\\d+[[:punct:]]\\d+"))
...
leaflet() %>% 
  addTiles() %>% 
  addMarkers(lng = i_sm$longitude, lat = i_sm$latitude)
```
...
```{r insc-sf, eval=TRUE}
i <- inscriptions %>%
  filter(!is.na(longitude)) %>% # remove NAs in the longitude column
  filter(!is.na(latitude)) %>% # remove NAs in the latitude column
  filter(longitude > 5 & longitude < 20) # only take longitude values above 5 and below 20
...
```{r}
# In order to find the ratio between urban to rural inscriptions I take the number of rows in the insc_urban which correponds to the number of inscriptions in the urban areas and divide it by the number of rows in the insc_rural which corresponds to the number of inscriptions in the rural aras:
nrow(insc_urban)/nrow(insc_rural)

# The ratio of urban to rural inscriptions is 6.6 which means that there are app. 6 times as many inscriptions in urban areas compared to rural areas. 
```
...
```{r overcounting and distance, eval=TRUE}
## Overcounting ##
clist <- st_intersects(cities_it, insc_sf3035)
length(unique(unlist(clist))) # 10462
which(!unique(unlist(clist))) # 0

## Average distance ##
dist_mat <- st_distance(insc_sf3035,
                        cities_sf3035[st_nearest_feature(insc_sf3035,cities_sf3035$geometry),],
                        by_element = TRUE)
summary(dist_mat)
```
...
*5. What is the average distance of all inscriptions from the nearest cities within the convex hull?*
