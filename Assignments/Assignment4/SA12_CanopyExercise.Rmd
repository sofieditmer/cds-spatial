...
# Take a quick look
head(tree_counts) # The geometry variable still exists
...
neighborhoods_counts <- mutate(neighborhoods_counts, tree_cnt = ifelse(is.na(tree_cnt), 0, tree_cnt)) # If NA return 0 if not NA return tree_cnt
...
ggplot(neighborhoods_avg_canopy, aes(x = neighborhoods_counts$tree_density)) + 
  geom_histogram(color = "white") +
  labs(title = "Total Tree Density", x = "Tree Density (tree count/area)")

# Create a histogram of average canopy (avg_canopy)
ggplot(neighborhoods_avg_canopy, aes(x = avg_canopy)) + 
  geom_histogram(color = "white") +
  labs(title = "Average Canopy", x = "Average canopy")

# Create a scatter plot of tree_density vs avg_canopy
ggplot(neighborhoods, aes(x = neighborhoods_counts$tree_density, y = neighborhoods_avg_canopy$avg_canopy)) + 
  geom_point() + 
  stat_smooth() +
  labs(title = "Tree Density and Average Canopy", x = "Tree density", y = "Average canopy")
  

# Compute the correlation between density and canopy
cor(x = neighborhoods_counts$tree_density, y = neighborhoods_avg_canopy$avg_canopy)
# Negative correlation of -0.34.
```
...
ggplot(neighborhoods) + 
  geom_sf(aes(fill = neighborhoods_counts$tree_density)) + 
  labs(title = "Map of tree Density in New York City")

# Plot the tree canopy with default colors
ggplot(neighborhoods) + 
  geom_sf(aes(fill = avg_canopy)) + 
  labs(title = "Map of tree canopy in New York City")
...
# I had to change the class of the avg_column before plotting
...
# Map 1: Manhattan and neighborhoods
map1 <- tm_shape(manhattan) + 
  tm_rgb() + 
  tm_shape(neighborhoods) + 
  tm_borders(col = "black", lwd = 0.5, alpha = 0.5) +
  tm_layout(main.title = "Aerial Map of Manhattan Neighborhoods in New York City",
            main.title.size = 0.8)

# Adding tree density to neighborhoods
neighborhoods$tree_density <- neighborhoods_counts$tree_density

# Tree density map
map2_density <- tm_shape(neighborhoods, bbox = manhattan) + 
    tm_polygons("tree_density", palette = "Greens", 
        style = "quantile", n = 7, 
        title = "Trees per sq. KM") +
...
# Canopy map
map2_canopy <- tm_shape(neighborhoods, bbox = manhattan) + 
    tm_polygons("avg_canopy", palette = "Greens", 
        style = "quantile", n = 7, 
        title = "Average tree canopy (%)") +
...
# Combine maps
tmap_arrange(map1, map2_density, map2_canopy, asp = NA)
```
