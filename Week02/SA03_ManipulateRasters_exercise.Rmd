...
# Read in the rasters
canopy <- raster("data/canopy.tif") # single-band raster
manhattan <- brick("data/manhattan.tif") # multi-band raster

# Get the extent (metadata) of the canopy object
extent(canopy)
...
inMemory(canopy) # FALSE - this means that it canopy values have not been read in
...
plotRGB(manhattan) 
# When using the plotRGB() function it means that we plot the many layers together so that the colors are combined
```
