...
When the geographic data you read in with `sf` already has a CRS defined both `sf` and `raster` will recognize and retain it. When the CRS is not defined you will need to define it yourself using either the EPSG number (numeric) or the proj4string (text string).
...
crs(manhattan_crs)
# Yes, they now have the same coordinate reference system (CRS) values
```
...
# Nope. This is because they do not share the same CRS value
...
# Yes, now we see the neighborhoods, because the two layers (neighborhoods and canopy) now share CRS-value
