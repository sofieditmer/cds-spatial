...
species_counts <- dplyr::count(trees, species, sort = TRUE)
species_counts
...
boro_distinct <- dplyr::distinct(boro_no_geometry)

# Join the county detail into the trees object
trees_with_county <- dplyr::inner_join(trees, boro_distinct, by = c("boroname" = "boro_name"))
...
object_size(neighborhoods) # 1.84 MB
...
sum(cnt_neighborhoods) # 210736 is the total number of vertices (i.e. the number of points required to delineate a line or polygon)
...
object_size(neighborhoods_simple) # 195 kB
...
sum(cnt_neighborhoods_simple) # 2908
...
# Confirm conversion
class(trees_sf)
# It has now been converted back to an sf object
```
