...
df_crs <- st_transform(df_sf, crs = crs(manhattan, as.text = TRUE))
df_crs

# Buffer the points
df_buf <- st_buffer(df_crs$geometry, dist = 1000)

# Plot the manhattan image (it is multi-band)
plotRGB(manhattan)
plot(df_buf, col = "firebrick", add = TRUE)
plot(df_crs, pch = 16, add = TRUE)
```
...
* Use the `length()` function from `base` R on `beech` and `beech1` to confirm that the number of features went from 8 to 1.
