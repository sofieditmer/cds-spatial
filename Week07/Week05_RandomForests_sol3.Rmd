...
plotRGB(kaze, stretch = "lin") # we need to stretch it so it fits into the spectrum

# West next
kazw <-brick("data/KazW.tif")
plotRGB(kazw, stretch = "his") # stretch = hist (more contrast)
...
# Band number 4 is the near-infrared
```

We can look at the pairs: 
```{r}
pairs(kazw)
pairs(kaze)
# We can see that the green and blue are correlated. There is less correlation with the near-infrared. 
```
Now, let's calculate the NDVI (showing vegetation health) using raster algebra or overlay
```{r}
# Extract bands 3 and 4 (Red and Near-Infrared)
kaze3 <- kaze[[3]] # extracting band 3
kaze4 <- kaze[[4]] # extracting band 4

# Calculating NDVI
ndvi <-  overlay(kaze4, kaze3, fun=function(x,y){(x-y)/(x+y)}) # we use the overlay function to calculate NDVI (we could also just have used raster algebra but this is slower)

plot(ndvi)
# Values below 0 (white): water, i.e. reservoirs, lakes etc.

# Around 0 (yellow): fields with no vegetation

# Green: fields with crops and/or forest
...
```{r}
# Creating a new brick out of all the layers we are interested in (green, red, near-infra, nvdi, and reclassified nvdi_0)
kaze2 <- kaze[[2]]
covs <- brick(kaze2,kaze3,kaze4,ndvi, nvdi_0)
plot(covs)
# Here we can see the layers in the brick we have just made
```
...
Before the random forest, let's check what an unsupervised classification can do, so we can see how much (little) random forest improves on it. We do this in order to have something to compare the random forest with. 
```{r}
...
```{r}
clusters <- raster(covs)   ## create an empty raster with same extent as kaz
...
# Now we can see forest, soil, cities, water etc. in the colors we have specified. 
```
...
library(sf)
trainingPoly <- st_read("data/Classification/kaz_training_aggr.shp") # we need a trainning dataset on which we can train.
...
classes <- rasterize(trainingPoly, ndvi, field='Class') # here we are rasterizing 
plot(classes) # this is a raster
...
```{r}
# Now we mask the image, to see what reflectance is behind the polygon
